...
now = time.time() #代码开始时间 time.time() 这个模块的核心之一就是time()，它会把从纪元开始以来的秒数作为一个float值返回。（1970年后的浮点秒数）
print (now)
...
f = linecache.getlines('t.txt')  #打开文件返回一个列表，包含文件所有行。
'''
  B = linecache.getlines('test.txt')[0:2]  # 打开文件的0,1两行作为一个列表  
  C = linecache.getline('test.txt', 4)  # 打开文件的第四行，这里面4表示第几行从1开始算起  
'''
lines = [x[1:-1].split('","') for x in f] #拆分   "," 是拆分标志(不是只有逗号，是",") 一行在一个[]里面 e.g第三行就是line[2]
#可以把x[1:-1]改成了x[1:-2],因为结尾除了‘\n’还有一个”，这样保证lines中每个数据都不带引号 
#[1:-1]是指定一行读多少 不是指定读几行 [1:-2]就相当于前后两个“ 没读
#print (type(lines))
#print (lines[5])
'''
f = open("t.txt","r")
...
'''
people = [ x[keys['username']] for x in lines ]  #people是一个存了username的列表
users = set(people)
'''
...
http://blog.csdn.net/bullpride/article/details/51952896
'''
